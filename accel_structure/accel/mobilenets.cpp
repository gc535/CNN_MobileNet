...
/*
void load_weights(const float* w, int size){
...
int loading_params(){
    ...
    load_weights(params.float_data(0), conv_weight_0);
}
*/
using namespace std;
...
int main(int argc,char **argv){
    ...
    Timer timer("software runtime");

    int img_count = atoi(argv[1]);
	int mean = atoi(argv[2]);
	int st_dev = atoi(argv[3]);

    int file_index = 0;
    string prefix;
    prefix = "/home/brandon/Cornell/image/resized_images/";
    string sufix;
    sufix = ".bmp";
    ...
    for(; file_index < img_count; file_index++){
        //set target file and casting char array to pass to the loadbmp function
        target = prefix + SSTR(file_index) + sufix;
        ...
        //////////////////////
        //float conv_weights[conv_weight_0];



        //////////////////////
    	timer.start();
        mobilenets(mem_conv1, mem_conv2);
        timer.stop();
    ...
    // can we stream out the ready result while computing the rest of the result?
    // is STL supported in Vivado? eg. vector?

void mobilenets(float* mem_conv1, float* mem_conv2){
    ////////////////////////////////////////////////
    //  We only have around 550KB memory on FPGA  //
    //  IN TOTAL: we can have 140,800 floats      //
    //  1. Weight cannot fit starting from 6th    //
    //     layer                                  //
    //                                            //
    ////////////////////////////////////////////////
    
    //layer 0
    // input feature map: 150,528
    // output feature map: 401,408
    // weights: 864

    //float block_inputfm[50176]; //size of a 224*224 


    //perform_conv2d(mem_conv1, mem_conv2, conv_weight_0, 112, 224, 32, 3, 3, 2);
    perform_conv2d_hw(mem_conv1, mem_conv2, conv_weight_0, 112, 224, 32, 3, 3, 2);
    ...
    logit_pointwise_conv2d(mem_conv1, mem_conv2, logit_conv_weight, 1001, 1024);
}
